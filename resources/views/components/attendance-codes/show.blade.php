                        ...
                        <table class="w-full h-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead
                                ...
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3">Event Day</th>
                                    <th scope="col" class="px-4 py-3">Student ID</th>
                                    <th scope="col" class="px-4 py-3">Attendee Name</th>
                                    <th scope="col" class="px-4 py-3">Status</th>
                                    <th scope="col" class="px-4 py-3">QR Image</th>
                                ...
                                @foreach ($attendanceCodes as $attendanceCode)
                                    <tr class="border-b dark:border-gray-700">
                                        <th scope="row"
                                            ...
                                            {{ $attendanceCode->id }}</th>
                                        <td class="px-4 py-3">
                                            {{ $attendanceCode->eventSession->name }}
                                        </td>
                                        <td class="px-4 py-3">
                                            {{-- {{ \Carbon\Carbon::parse($eventSession->time_start)->format('h:i A') }} --}}
                                            {{ $attendanceCode->event->name }}
                                        </td>

                                        {{-- <td class="px-4 py-3"><a href="/portfolio/{{ $application->applicant_id }}" target="_blank">Check</a></td> --}}
                                        <td class="px-4 py-3">
                                            {{-- {{ \Carbon\Carbon::parse($eventSession->time_end)->format('h:i A') }} --}}
                                            {{ $attendanceCode->eventSession->event_day }}
                                        </td>
                                        <td class="px-4 py-3">
                                            {{-- {{ $eventSession->is_accepting_attendance == true ? 'Open' : 'Close' }} --}}
                                            {{ $attendanceCode->attendee->student_id }}
                                        </td>
                                        <td class="px-4 py-3">
                                            {{-- {{ $eventSession->is_accepting_attendance == true ? 'Open' : 'Close' }} --}}
                                            {{ $attendanceCode->attendee->first_name }}
                                        </td>
                                        {{-- @php
                                        ...
                                        <td class="px-4 py-3">
                                            ...
                                            @else
                                                No record available
                                            @endif --}}
                                            @if (isset($attendanceCode->qrCodeSvg))
                                                {!! $attendanceCode->qrCodeSvg !!}
                                            @else
                                        ...
                                        <td class="px-4 py-3">
                                            ...
                                            {{ \Carbon\Carbon::parse($attendanceCode->created_at)->format('h:i A') }}
                                        </td>
                                        <td class="flex items-center  px-4 py-3">
                                            {{-- <x-events.update :eventSession="$eventSession" type="session" />
                                            <a href="/attendance-codes/{{ $eventSession->id }}" target="_blank"
