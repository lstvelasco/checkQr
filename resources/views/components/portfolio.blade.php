<div class="flex justify-center items-center min-h-screen max-w-screen-xl px-4 py-8 mx-auto lg:px-6 sm:py-16 lg:py-24">
  <div class="max-w-2xl mx-auto text-center">
      <img class="m-auto mb-2 rounded-full w-36 h-36"
          src="{{ $user->avatars()->where('isActive', true)->first()?->image_url ? '/storage/' . $user->avatars()->where('isActive', true)->first()->image_url : '/storage/avatars/default.png' }}"
          alt="Extra large avatar">
      <h2 class="text-3xl font-extrabold leading-tight tracking-tight text-gray-900 sm:text-4xl dark:text-white">
          Hey! I'm {{ $user->first_name }}
      </h2>
      <h6 class="text-lg font-bold dark:text-white">{{ $user->account_type }}</h6>
      <h6 class="text-lg font-bold dark:text-white">{{ $user->role }}</h6>
      <h6 class="text-lg font-bold dark:text-white">BSI/T {{ $user->year . ' ' . $user->section  }}</h6>
      <p class="mt-4 text-base font-normal text-gray-500 sm:text-xl dark:text-gray-400">
          I love MaRSU!
          <br>
          <span class="italic">I love CICS!</span>
      </p>
  </div>
