...
@if ($type == 'event')
    ...
    <div class="fixed top-13 right-4">
        <button id="updateAnnouncementButton"
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function(event) {
            document.getElementById('updateAnnouncementButton').click();
        });
    </script>
@elseif ($type == 'session')
    @props(['event', 'eventSession'])
    <!-- drawer init and show -->
    {{-- <div class="fixed top-13 right-4"> --}}
    <div>
        <button id="updateAnnouncementButton"
            ...
            type="button" data-drawer-target="{{ $eventSession->id }}drawer-update-announcement-default"
            data-drawer-show="{{ $eventSession->id }}drawer-update-announcement-default"
            aria-controls="{{ $eventSession->id }}drawer-update-announcement-default">
            Update
        </button>
    </div>
    <!-- drawer component -->
    <div id="{{ $eventSession->id }}drawer-update-announcement-default"
        class="fixed top-0 left-0 z-50 w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform -translate-x-full bg-white dark:bg-gray-800"
        tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
        <h5 id="drawer-label"
            ...
            Update
            Session</h5>
        <button type="button" data-drawer-dismiss="{{ $eventSession->id }}drawer-update-announcement-default"
            aria-controls="{{ $eventSession->id }}drawer-update-announcement-default"
            class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
            <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                xmlns="http://www.w3.org/2000/svg">
        ...
        <form action="/events/{{ $eventSession->event->id }}/{{ $eventSession->id }}" method="POST">
            @csrf
            @method('POST')
            <div class="space-y-4">
                <div hidden class="hidden">
                    ...
                    <input type="number" name="event_id" id="event_id"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        placeholder="Type event id" required="" value="{{ $eventSession->event->id }}">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        placeholder="Type event name" value="{{ $eventSession->name }}" required="">
                    <x-form-error name="name"></x-form-error>
                </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="time" id="time_start" name="time_start"
                                ...
                                min="00:00" max="24:00"
                                value="{{ \Carbon\Carbon::parse($eventSession->time_start)->format('H:i') }}"
                                required />
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="time" id="time_end" name="time_end"
                                class="bg-gray-50 border leading-none border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                min="00:00" max="24:00"
                                value="{{ \Carbon\Carbon::parse($eventSession->time_end)->format('H:i') }}"
                                required />
                ...
                <div>
                    ...
                    <input type="text" name="event_day" id="event_day"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        placeholder="Day 1, Day 2,.." value="{{ $eventSession->event_day }}">
                    ...
                    <div class="flex justify-between items-center mb-3">
                        ...
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" value="1" name="is_accepting_attendance"
                                ...
                                {{ $eventSession->is_accepting_attendance ? 'checked' : '' }}>
