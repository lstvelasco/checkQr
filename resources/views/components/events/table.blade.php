...
@if (Auth::user()->role == 'Attendee')
    <div class="w-full min-h-[61vh] md:min-h-[65vh] mx-auto ">
        <!-- Start coding here -->
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
            ...
            <div class="overflow-x-auto min-h-[61vh] md:min-h-[65vh]">
                <table class="w-full h-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">Time Start</th>
                            <th scope="col" class="px-4 py-3">Time End</th>
                            <th scope="col" class="px-4 py-3">Status</th>
                            <th scope="col" class="px-4 py-3">Attendance</th>
                            <th scope="col" class="px-4 py-3">Attendance Time</th>
                    ...
                    <tbody>
                        @foreach ($event->sessions as $eventSession)
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3"> {{ $eventSession->time_start }} </td>
                                {{-- <td class="px-4 py-3"><a href="/portfolio/{{ $application->applicant_id }}" target="_blank">Check</a></td> --}}
                                <td class="px-4 py-3"> {{ $eventSession->time_end }} </td>
                                <td class="px-4 py-3">
                                    {{ $eventSession->is_accepting_attendance == true ? 'Open' : 'Close' }} </td>
                                @php
                                ...
                                <td class="px-4 py-3">
                                    @if ($attendanceRecord && $attendanceRecord->is_present == true)
                                        Attended
                                    @elseif ($attendanceRecord && $attendanceRecord->is_present == false)
                                        Missed
                                    @else
                                        No record
                                    @endif
                                ...
                                <td class="px-4 py-3">
                                    @if ($attendanceRecord)
                                        {{ $attendanceRecord->validator_id == null ? 'System' : $attendanceRecord->validator->first_name . ' ' . $attendanceRecord->validator->last_name }}
                                    @else
                                        No record
                                    @endif
                                </td>
                                <td class="flex items-center  px-4 py-3">
                                    @if ($eventSession->is_accepting_attendance == true)
                                        <a href="/events/{{ $event->id }}/{{ $eventSession->id }}" target="_blank"
                                            rel="noopener noreferrer">
                                            <button id="scanqr-button"
                                    ...
                                    @else
                                        <button id="scanqr-button" disabled
                                            class="text-white cursor-not-allowed bg-primary-700 shadow-lg hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800"
                                            type="button">
                                            Scan QR
                                        </button>
                                    @endif
...
@elseif (Auth::user()->role == 'Attendance Officer')
    <div class="w-full min-h-[61vh] md:min-h-[65vh] mx-auto text-white">
        <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
            {{-- <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
            <div class="w-full md:w-1/2">
                <form class="flex items-center">
                ...
                <table class="w-full h-full text-sm text-left text-gray-500 dark:text-gray-400">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col" class="px-4 py-3">Codes Generated</th>
                            <th scope="col" class="px-4 py-3">
                                <span class="sr-only">Actions</span>
                                Actions
                            </th>
                    ...
                    <tbody>
                        @foreach ($event->sessions as $eventSession)
                            <tr class="border-b dark:border-gray-700">
                                ...
                                <td class="px-4 py-3">
                                    {{ \Carbon\Carbon::parse($eventSession->time_start)->format('h:i A') }} </td>

                                {{-- <td class="px-4 py-3"><a href="/portfolio/{{ $application->applicant_id }}" target="_blank">Check</a></td> --}}
                                <td class="px-4 py-3">
                                    {{ \Carbon\Carbon::parse($eventSession->time_end)->format('h:i A') }} </td>
                                <td class="px-4 py-3">
                                    {{ $eventSession->is_accepting_attendance == true ? 'Open' : 'Close' }} </td>
                                @php
                                    $attended = $attendances->where('is_present', true)->count();
                                @endphp
                                ...
                                <td class="flex items-center  px-4 py-3">
                                    ...
                                    <a href="/attendance-codes/{{ $eventSession->id }}" target="_blank" rel="noopener noreferrer">
                                        <button
                                            class="z-100 text-white bg-primary-700 shadow-lg hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 ml-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800"
                                            type="button">
                                            Codes
                                        </button>
                                    </a>
